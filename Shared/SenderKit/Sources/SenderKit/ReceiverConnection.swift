...
public actor ReceiverConnection {
    ...
    // Callers observe this by polling rather than a push handler — that
    // sidesteps having to capture a non-Sendable observer (a SwiftUI view
    // model) in a @Sendable closure just to hop back to MainActor.
